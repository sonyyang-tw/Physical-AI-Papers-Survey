...
[Spatially Aware World Action Model via Geometric Latent Diffusion]({{ site.baseurl }}/wm/spatially-aware-world-action-model-via-geometric-latent-diffusion-1946673367/) — World Action Models (Video-Action Joint Modeling) — arXiv:2609.02531, new on 2026-09-02, not previously logged on this page
...
[DreamDojo: A Generalist Robot World Model from Large-Scale Human Videos]({{ site.baseurl }}/wm/dreamdojo-a-generalist-robot-world-model-from-large-scale-human-videos-1955665089/) — Robot-Specific Action-Conditioned World Models (Category 2, also touches data scaling) — arXiv:2602.06949, submitted 2026-02-06, top-conference-caliber (led by NVIDIA GEAR, with authors including Jitendra Malik, Pieter Abbeel, Yuke Zhu, and Linxi "Jim" Fan), not previously logged on this page
...
[Inference-time Physics Alignment of Video Generative Models with Latent World Models]({{ site.baseurl }}/wm/inference-time-physics-alignment-of-video-generative-models-with-latent-world-mo-1959634541/) — World Model Evaluation & Physical-Reasoning Benchmarks (Category 5, also relevant to the foundational model V-JEPA 2 as a downstream application) — arXiv:2601.10553, submitted 2026-01-15 (v2 revised 2026-02-27), top-conference-caliber (winner of the ICCV 2025 Perception Test PhysicsIQ Challenge), not previously logged on this page
...
[Pelican-Sim 1.0: A General World Model Simulator for Embodied Intelligence]({{ site.baseurl }}/wm/pelican-sim-1-0-a-general-world-model-simulator-for-embodied-intelligence-1968596049/) — Robot-Specific Action-Conditioned World Models (Category 2) — arXiv:2609.12036, new on 2026-09-10, a new same-day arXiv release (submitted 5 days prior)

Why selected: the author group is from AgiBot's Beijing Innovation Center of Humanoid Robotics (the same institution as this page's already-covered τ0-WM), meeting the "major tech company/well-known lab" criterion; its "Action-visual injection" (bridging actions and pixels via URDF- and camera-rendered action videos) paired with sparse MoE layers to absorb heterogeneous embodiment-dynamics conflict is the first systematic architectural design to simultaneously address cross-embodiment generality and action controllability — a genuine methodological innovation rather than a minor recombination of existing tricks; trained on roughly 1 million real and simulated trajectories, and validated across three independent benchmarks (AgiBotWorld Beta, RoboMIND, RoboTwin) and four downstream applications.

Pelican-Sim 1.0 addresses the problem that existing robot world models struggle to generalize across heterogeneous embodiments, with insufficient coupling between action conditioning and pixel prediction: it represents cross-embodiment actions via a unified 28-dimensional action-value space, injects URDF/camera-rendered action videos into a 28-layer Video DiT backbone (Context Block injects action-video residuals, action-value embeddings perform scale-and-shift modulation), aided by sparse MoE layers, then compresses the 35-step diffusion rollout into 4-step autoregressive generation via causal adaptation and few-step distillation (5.67x speedup). Key results: PSNR improves by 4.636, 2.080, and 10.343 over the strongest baseline on three benchmarks respectively; downstream, adding 500 generated trajectories per task on RoboTwin raises policy success rate from 70% to 93%, with a policy-evaluation Pearson correlation of 0.994. Main limitation: as a technical report, it has not yet indicated a formal conference acceptance status, quantitative validation of the four downstream applications is limited to the single RoboTwin benchmark, and whether the distilled 4-step autoregressive simulator accumulates error over long-horizon rollouts is not fully discussed.

### Push Date: 2026-09-16

[GE-Act 2.0: Pretraining and Scaling a World-Action Model for Robotic Manipulation]({{ site.baseurl }}/wm/ge-act-2-0-pretraining-and-scaling-a-world-action-model-for-robotic-manipulation-1972876063/) — Robot-Specific Action-Conditioned World Models (Category 2, also touches data scaling) — arXiv:2609.05588, new on 2026-09-04, an AgiBot Research Team technical report (top-conference-caliber not yet formally noted), not previously logged on this page

Why selected: the author group is the AgiBot Research Team (a large industrial team of 46 authors), meeting the "major tech company/well-known lab" criterion, and the same institution as this page's already-covered τ0-WM, being the direct successor to the Genie Envisioner series; methodologically, it is the first to systematically build a WAM entirely from scratch (rather than inheriting an existing pretrained video generator), using a decoupled CoAE/SVP/IDM three-module architecture paired with knowledge-aligned selective optimization (KASO) to validate a data-scaling law — a clear architectural and empirical departure from existing "inherited" WAM lines rather than a minor recombination of existing tricks; evaluation spans 100 tasks, 20 skill groups, and held-out scenes/backgrounds/lighting/objects, with data scale spanning 300 to 30,000 hours, representing an industrial-scale investment. While it has not yet noted a formal conference acceptance, its methodological rigor and scale meet a top-conference-caliber standard.

GE-Act 2.0 addresses the problem that WAM-specific pretraining and data-scaling laws have never been systematically validated: it trains a control-oriented autoencoder (CoAE), a single-step visual planner (SVP), and an inverse dynamics model (IDM) from scratch, jointly trained via KASO which filters out predicted futures inconsistent with recorded action behavior. Key results: in a zero-shot (no per-task fine-tuning) setting, scaling the co-training data from 300 to 30,000 hours raises G1-OP success rate from 17.1% to 44.1% and G2-90D from 13.4% to 31.1% (improving 17.7 points despite making up less than 2% of the data, showing cross-embodiment transfer), with skill coverage correlating strongly with OOD success (Pearson r=0.80). Main limitation: the technical report has not yet noted a formal conference acceptance status, and all evaluation is based on AgiBot's own platform, with cross-institution third-party validation still needed.
